...
#Program to predict Vp using Silicon content, temprature & Pressre
import pandas as pd
import matplotlib.pyplot as plt

dataset = pd.read_csv("Data_Cal_Si.csv")
X = dataset.iloc[:, 1:4]    #3 Features
Y = dataset.iloc[:, 4]   #Vp target values
...
model = Sequential()
model.add(Dense(8, input_dim=3, activation='tanh', kernel_initializer='he_uniform'))
...
new_prediction = model.predict(sc.transform(np.array([[64.6,	20,	600]])))
...
val_data = pd.read_csv("Data_Val_Si.csv")
X_val = val_data.iloc[:, 1:4]    
Y_val = val_data.iloc[:, 4]
